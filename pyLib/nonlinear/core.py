...
class System_Function:
  ...
  def __init__(self,
               f: Callable[[np.ndarray, List[float]], np.ndarray],
               type: int,
...
class SystemDescriptor:
  ...
  def add_function(
      self,
      function: Callable[[np.ndarray, List[float]], np.ndarray],
      function_type: int,
...
class EquPointAnalisys:
  ...
  def __init__(self, jac: Callable[[np.ndarray, List[float]], np.ndarray],
...
def jacobian(fun: Callable[[np.ndarray, List[float]], np.ndarray],
  ...
  def f(y, h):
    res = (fun(y + h, params) - fun(y - h, params)) / (2 * step)  # type: ignore
...
def transition_matrix(system: SystemDescriptor, x0: np.ndarray,
                      params: List[float], T: float, step: float) -> np.ndarray:
